...
Design choices for the **primary trusted SSID** on the **UniFi U7 Lite**, with reasoning. This is not a step-by-step UniFi manual—only **what** is configured and **why**. Operational SSID names and VLAN bindings stay private; see [portfolio scope](portfolio-scope.md).

## Configuration

- **Encryption:** **WPA3** enabled manually on the first (main) WLAN used for **general household** clients—phones, laptops, and shared use.
- **Scope:** This SSID is **not** reserved for IoT. It is the **default trusted WiFi** for primary clients.
...
1. **Modern baseline** — WPA3 is the appropriate default for **current** client devices in favor of WPA2-only on the main network.
