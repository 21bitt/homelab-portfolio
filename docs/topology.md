...
**Access:** A UniFi switch uplinks to the firewall (trunk if multiple VLANs, or a flat LAN during early setup). A UniFi WiFi 6 access point hangs off the switch with PoE. SSIDs map to VLANs so wireless clients land in the same trust zones as wired ones.
